...
type ServeTargets = {
  protein_serves: number | null
  carb_serves: number | null
  fat_serves: number | null
  fruit_serves: number | null
  veg_unlimited: boolean | null
  notes: string | null
} | null
...
function fmtServes(v: number | null | undefined) {
  if (v == null) return '0'
  return Number.isInteger(v) ? String(v) : v.toFixed(1).replace(/\.0$/, '')
}

export default function PrintClientCheatSheet({
  clientId,
  clientName,
}: {
  clientId: string
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([fd, td]) => {
        setTagged(fd.foods ?? [])
        setOrgName(fd.org_managed?.org_name ?? null)
        setTargets(td.targets ?? null)
      })
  ...
  const hasTargets = !!targets && (
    (targets.protein_serves ?? 0) > 0 ||
    (targets.carb_serves ?? 0) > 0 ||
    (targets.fat_serves ?? 0) > 0 ||
    (targets.fruit_serves ?? 0) > 0 ||
    targets.veg_unlimited ||
    !!targets.notes
  )
    ...
    <div className="min-h-screen bg-gray-100 print:bg-white">
      <style>{`
        ...
        @media print {
          ...
          .targets-block { break-inside: avoid; page-break-inside: avoid; }
      ...
      <div className="no-print sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-3 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-gray-500">Preview for <strong>{clientName}</strong> — choose <strong>Save as PDF</strong> in the print dialog.</p>
        ...
        <div className="page-card bg-white rounded-2xl shadow-sm border border-gray-200 p-8 print:p-0">
          ...
          <div className="flex items-start justify-between gap-6 pb-5 border-b border-gray-200">
            <div className="min-w-0">
              <p className="text-xs uppercase tracking-wider text-gray-400 font-semibold">Personalised Serve Guide</p>
              <h1 className="text-2xl font-bold text-gray-900 mt-1">{clientName}</h1>
              {orgName && (
                <p className="text-xs text-gray-500 mt-1">Cheat sheet shared by {orgName}</p>
              )}
          ...
          {/* Daily targets */}
          {loaded && hasTargets && targets && (
            <div className="targets-block mt-5 mb-5 rounded-2xl border border-blue-200 bg-blue-50 px-5 py-4">
              <p className="text-xs uppercase tracking-wider text-blue-700 font-semibold mb-3">Your daily serve targets</p>
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-pink-600 font-semibold">Protein</p>
                  <p className="text-xl font-bold text-gray-900">{fmtServes(targets.protein_serves)}</p>
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-green-600 font-semibold">Fats</p>
                  <p className="text-xl font-bold text-gray-900">{fmtServes(targets.fat_serves)}</p>
                  <p className="text-[10px] text-gray-500">serves / day</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-orange-500 font-semibold">Fruit</p>
                  <p className="text-xl font-bold text-gray-900">{fmtServes(targets.fruit_serves)}</p>
                  <p className="text-[10px] text-gray-500">serves / day</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-emerald-600 font-semibold">Veg</p>
                  <p className="text-xl font-bold text-gray-900">{targets.veg_unlimited ? '∞' : '0'}</p>
                  <p className="text-[10px] text-gray-500">{targets.veg_unlimited ? 'unlimited' : 'see notes'}</p>
                </div>
              </div>
              {targets.notes && (
                <div className="mt-3 pt-3 border-t border-blue-100">
                  <p className="text-[10px] uppercase tracking-wider text-blue-700 font-semibold mb-1">Notes from your coach</p>
                  <p className="text-sm text-gray-800 whitespace-pre-wrap">{targets.notes}</p>
                </div>
              )}
            </div>
          )}
          ...
          {loaded && !hasTargets && (
            <div className="mt-5 mb-5 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-3">
              <p className="text-sm text-amber-800">
                <span className="font-semibold">No serve targets set yet.</span> Set targets in the Serve Guide tab to personalise this PDF.
              </p>
            </div>
          )}

          {loaded && tagged.length === 0 && (
            <p className="py-10 text-center text-sm text-gray-400 italic">No foods tagged in the cheat sheet yet.</p>
          )}
          ...
          <div className="mt-8 pt-4 border-t border-gray-200 flex items-center justify-between text-[11px] text-gray-400">
            ...
            <span>{clientName} · Serve Guide</span>
          </div>
        </div>
      </div>
    </div>
