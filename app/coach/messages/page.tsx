...
export default async function CoachMessagesPage() {
  ...
  const { data: unreadMsgs } = await supabase
    ...
    .in('conversation_id', (convos ?? []).map((c) => c.id).length ? (convos ?? []).map((c) => c.id) : ['none'])
  ...
  return (
    <main className="flex-1 flex flex-col max-w-2xl w-full">
      <div className="bg-white border-b px-6 py-4">
        ...
        <p className="text-xs text-gray-400 mt-0.5">{convos?.length ?? 0} conversation{convos?.length !== 1 ? 's' : ''}</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {(!convos || convos.length === 0) && (
          <div className="p-10 text-center">
            ...
            <p className="text-gray-400 text-sm mt-1">Message a client from their profile page.</p>
          </div>
        )}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between mt-0.5">
                  <p className={`text-xs truncate ${unread ? 'text-gray-700 font-medium' : 'text-gray-400'}`}>
                    {latest
                      ? `${latest.sender_id === coachId ? 'You: ' : ''}${latest.body}`
                      : 'No messages yet'}
                  </p>
                  {unread > 0 && (
